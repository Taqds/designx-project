...
def fix_content(content):
    ...
    # 2. Universal Navigation and Global Layout Styling
    nav_style = """
        /* Universal Navigation & Responsive Fixes */
        nav {
            position: fixed;
            top: 0;
            left: 0;
    ...
    # Replace existing Universal styling if it exists, or common nav styles
    if '/* Universal Navigation & Responsive Fixes */' in content:
        content = re.sub(r'/\* Universal Navigation & Responsive Fixes \*/.*?@media \(max-width: 900px\) \{.*?\}', nav_style, content, flags=re.DOTALL)
    elif '/* Universal Navigation Styling */' in content:
        content = re.sub(r'/\* Universal Navigation Styling \*/.*?@media \(max-width: 900px\) \{.*?\}', nav_style, content, flags=re.DOTALL)
    else:
    ...
    # 4. Hero Heading Response (Clamp fix)
    content = re.sub(r'font-size:\s*clamp\(2\.2rem', 'font-size: clamp(1.8rem', content)
    
    # 5. Correct Grids (Fallback for non-dynamic styles)
